...
describe("GET /api/venue-ratings", () => {
  ...
  it("returns aggregate counts and the authenticated user's rating", async () => {
    ...
    const json = await res.json();
    expect(json.upCount).toBe(2);
    expect(json.downCount).toBe(1);
    expect(json.userRating).toBe("down");
    expect(json.data).toEqual({ upCount: 2, downCount: 1, userRating: "down" });
  });
  ...
  it("lets guests read counts without a user rating", async () => {
    ...
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json).toMatchObject({ upCount: 1, downCount: 0, userRating: null });
...
describe("POST /api/venue-ratings", () => {
  ...
  it("upserts the user's venue rating", async () => {
    ...
    const res = await POST(
      request("POST", "http://localhost/api/venue-ratings", { venueId: "venue-1", rating: "up" }),
    );
    ...
    const json = await res.json();
    expect(json.data).toEqual({ venueId: "venue-1", rating: "up" });
  });
});
