...
export default async function SharePage({ searchParams }: SharePageProps) {
  ...
  return (
    <div className="min-h-[calc(100vh-5rem)] bg-[#0A0A0F] px-5 py-10 text-white">
      <section className="mx-auto flex min-h-[520px] w-full max-w-md flex-col justify-center">
        <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#00F5D4]">
          NightVibe
        </p>
        ...
        {sharedContent.length > 0 && (
          <div className="mt-8 rounded-lg border border-white/10 bg-white/[0.04] p-4">
            ...
            {url && (
              <p className="mt-3 break-words text-xs font-medium text-[#00F5D4]">
                {url}
              </p>
            )}
          </div>
        ...
        <Link
          href="/map"
          className="mt-8 inline-flex h-12 items-center justify-center rounded-md bg-[#00F5D4] px-5 text-sm font-semibold text-[#0A0A0F] transition hover:bg-[#66ffea] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
        >
          Open map
        </Link>
      </section>
