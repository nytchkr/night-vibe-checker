...
export function getMapViewportStyle(): CSSProperties {
  return {
    height: process.env.NEXT_PUBLIC_ENV === "development" ? "calc(100dvh - 100px)" : "calc(100dvh - 80px)",
    minHeight: "520px",
  };
}
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center px-4">
        <div className="flex flex-col items-center gap-4 rounded-2xl border border-white/10 bg-[#0A0A0E]/80 px-7 py-6 shadow-2xl backdrop-blur">
          <span className="h-10 w-10 animate-spin rounded-full border-2 border-white/15 border-t-[#8B6CFF]" aria-hidden="true" />
